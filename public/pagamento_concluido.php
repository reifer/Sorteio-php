...
// Buscar participante confirmado
...
$result = $stmt->execute();
$participante = $result->fetchArray(SQLITE3_ASSOC);
...
<body>
  ...
  <script>
    ...
    const nome = <?= json_encode($participante['nome']) ?>;
    const telefone = <?= json_encode($telefone) ?>;
    const numeros = <?= json_encode($numeros) ?>;
    const valor = <?= json_encode(number_format($participante['valor'], 2, ',', '.')) ?>;

    const mensagem = `Olá ${nome}! 🎉 Seu pagamento foi confirmado.\n\nVocê recebeu os seguintes números da rifa:\n${numeros.join("\n")}\n\nValor: R$ ${valor}\n\nBoa sorte no sorteio! 🍀`;
